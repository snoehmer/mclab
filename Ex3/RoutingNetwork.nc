...
interface RoutingNetwork
{
	...
	command result_t sendCommandMsg(uint16_t destination_id, uint16_t command_id, uint16_t argument);
	...
	event result_t receivedCommandMsg(uint16_t sender_id, uint16_t command_id, uint16_t argument);
}
